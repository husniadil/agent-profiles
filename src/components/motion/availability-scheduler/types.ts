...
// means re-applying this change. See the fuller note in `day-row.tsx`.
//
// The block labels times in 12-hour form (`label12`, "9:00 AM"). This window is
// translated into six languages, most of which write a 24-hour clock, so the
// options are labelled `HH:MM` instead — which is also exactly the value they
// carry, hence the identity below rather than a second formatter to keep in step.
...
/// LOCALLY MODIFIED (was `label12`, "9:00 AM"): a 24-hour clock reads the same
/// in every language this window speaks. `toValue` already produces `HH:MM`, so
/// the label a time carries is the value itself.
...
/// LOCALLY MODIFIED: exported. A stored time need not sit on the current grid —
/// a schedule saved at `step={15}` keeps 09:15 after the step becomes 10 — and a
/// value with no matching option leaves the field showing its placeholder rather
/// than the time it holds. `day-row` calls this so the current value is always
/// among the options, which is what the block's own range helpers use it for.
