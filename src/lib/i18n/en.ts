...
export const en = {
  ...
  "schedule.caveat":
    "Wakes a sleeping Mac and opens the app at each day's time, only if you're logged in — it wakes about a minute early and works on battery, but a Mac that's fully shut down stays off, and a closed lid needs an external display for the app to actually open. Wakes are scheduled a few weeks ahead, so open Agent Profiles now and then to keep them going.",
