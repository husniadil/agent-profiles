...
- **The menu bar icon is the app's own mark rather than the letters `ap`.** Two stacked cards, one profile tucked behind another — the same figure the app icon and the banner already use. It was the wordmark because a filled card flattens to a solid block in a template image, where only the alpha channel survives; drawing both cards as outlines and cutting the one behind away from the one in front answers that, and puts the icon in the line weight every other icon on the menu bar is drawn in. The 7° tilt of the larger marks is dropped at this size: it comes to about a pixel of lean, which reads as a rasterising fault rather than as a card set down askew.
