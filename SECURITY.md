...
- **The update check** is the only request this app makes on its own: the auto-updater asking whether a newer release exists — on by default, switched off in Settings → General. It runs **once per app launch**, whether or not you ever open the window, and once more each time you press **Check now**. The configured endpoint is on `github.com`, but GitHub redirects it to `release-assets.githubusercontent.com`, and the manifest points the bundle download at `api.github.com` — so a firewall allowlist naming only the first host leaves a broken updater, surfacing as nothing more than "Couldn't check for updates". What the request discloses: your IP address, as any HTTP request does, and a `tauri-plugin-updater/2.10.1` user agent. What it does not: the version you are on or the platform you are running, since the endpoint is a fixed URL with none of the plugin's `{{current_version}}` or `{{target}}` placeholders. When a newer release exists it is downloaded, installed, and the app relaunches, without asking first.
...
That verification key is a **minisign key pair used only by the updater, and is not OS code signing**. It proves the bundle came out of this project's release workflow; it does nothing to make the result verifiable to macOS Gatekeeper or Windows SmartScreen. A build installed by the auto-updater is exactly as unsigned as one downloaded by hand, and the "damaged"/SmartScreen warnings described in the README are unchanged by it.
